...
function AutocompleteTambom(props) {
  ...
  const COMMENTS_SUBSCRIPTION = gql`
    subscription MySubscription($order_by: [isr_car_brand_order_by!]!, $where: isr_car_brand_bool_exp!) {
      isr_car_brand(order_by: $order_by, where: $where) {
        brand_image
        brand_name
        brand_id
      }
  ...
  useEffect(() => {
    if (subData) {
      let provinceList = subData.isr_car_brand;
      if (provinceList) {
        let pluckData = _.pluck(provinceList, "brand_name");
        ...
        renderInput={(params) => {
          return (
            <TextField
              ...
              label={option?.label == "" ? "" : option?.label || "เลือกยี้ห้อรถยนต์"}
              size="small"
              InputProps={{
                ...params.InputProps,
                style: option?.style || {},
