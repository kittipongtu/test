...
function AutocompleteTambom(props) {
  ...
  const COMMENTS_SUBSCRIPTION = gql`
    subscription MySubscription($where: postcode_bool_exp!, $order_by: [postcode_order_by!]!, $limit: Int!) {
      postcode(where: $where, order_by: $order_by, limit: $limit, distinct_on: zipcode) {
        id
        zipcode
      }
    }
  ...
  const handleInputChange = (event, newValue) => {
    let newWhere = { ...where };
    newWhere.zipcode = { _ilike: `%${newValue}%` };
  ...
  useEffect(() => {
    if (subData) {
      ...
      if (postcodeList) {
        let pluckData = _.pluck(postcodeList, "zipcode");
        ...
        renderInput={(params) => {
          return (
            <TextField
              ...
              label={option?.label == "" ? "" : option?.label || "เลือกรหัสไปรษณีย์"}
              size="small"
              InputProps={{
                ...params.InputProps,
                style: option?.style || {},
